...
import os, sys, time, json, logging, subprocess, argparse, re, urllib.request, urllib.error
...
DOCKER_IMAGE_DEFAULT = os.getenv("DOCKER_IMAGE_DEFAULT", "ghcr.io/astranero/wan2s2v:latest")
...
CURL_BASE = [
    "curl", "-4", "--silent", "--show-error", "--fail",
    "--connect-timeout", "20",
    "--header", f"Authorization: Bearer {VAST_API_KEY}",
    "--header", "Content-Type: application/json",
]

os.environ.update({k: "" for k in ("HTTPS_PROXY","HTTP_PROXY","https_proxy","http_proxy")})
...
def list_offers() -> List[Offer]:
    ...
    data = curl_json(["--request", "PUT", "--data", json.dumps(query), f"{BASE_URL}/search/asks/"])
    offers = []
...
def parse_external_port(inst: Dict[str, Any], internal: int, proto: str = "tcp") -> Optional[int]:
    """
    ...
    for key in ("port_mapping", "port_map", "ports_list", "port_list"):
        ...
        if isinstance(lst, list):
            for p in lst:
                try:
                    if int(p.get("internal")) == internal and str(p.get("proto","tcp")).lower() == proto:
                        ext = p.get("external") or p.get("external_port") or p.get("public")
                        if ext is not None:
                            return int(ext)
                except Exception:
                    continue
...
def wait_port_and_health(ip: str, inst: Dict[str, Any], internal_port: int, timeout: int = HEALTH_TIMEOUT) -> int:
    """
    ...
    while True:
        try:
            with urllib.request.urlopen(f"http://{ip}:{ext_port}{HEALTH_PATH}", timeout=5) as resp:
                if resp.status == 200:
                    LOG.info("Health OK")
                    return ext_port
...
def run_remote_inference(ip: str, port: int, audio_path: str, prompt: str) -> str:
    cmd = [
        "curl", "-sS", "-X", "POST",
        f"http://{ip}:{port}/s2v",
        "-F", f"prompt={prompt}",
...
def cli():
    ...
    ap.add_argument("--template-hash", default=TEMPLATE_HASH, help="Vast template hash to use")
    ap.add_argument("--image", default=DOCKER_IMAGE_DEFAULT, help="Fallback image if no template is provided")
    ap.add_argument("--keep", action="store_true", help="Keep instance running after inference")
    args = ap.parse_args()
    ...
    try:
        ...
        instance_id = create_instance(offer.id, template_hash=args.template_hash, image=args.image)
        ...
        out = run_remote_inference(ip, ext_port, args.audio, args.prompt)
        print(out)
